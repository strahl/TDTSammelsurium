function Sammelsurium_SoundOut(RP,TDT_fs)
   if (invoke(RP,'LoadCOFsf','Sammelsurium_SoundOut.rcx',TDT_fs) == 0)
      error('Could not load Sammelsurium_SoundOut.rcx');
   end;
   disp('Sammelsurium_SoundOut.rcx was loaded. Samplingrate was set to 25kHz.');
   if (invoke(RP,'Run') == 0)
   ...
   if (invoke(RP,'WriteTagVex','WaveformL',0,'F32',sound(1,:)) == 0) % size is  4e6 samples in Sammelsurium_SoundOut.rcx
