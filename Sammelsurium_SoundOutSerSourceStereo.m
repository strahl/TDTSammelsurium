function Sammelsurium_SoundOutSerSourceStereo(RP,TDT_fs)
   if (invoke(RP,'LoadCOFsf','Sammelsurium_SoundOutSerSourceStereo.rcx',TDT_fs) == 0)
      error('Could not load Sammelsurium_SoundOutSerSourceStereo.rcx');
   end;
   disp('Sammelsurium_SoundOutSerSourceStereo.rcx was loaded.');
   if (invoke(RP,'Run') == 0)
   ...
   if (invoke(RP,'WriteTagVex','WaveformL',0,'F32',sound(1,:)) == 0) % size is  4e6 samples in Sammelsurium_SoundOutSerSourceStereo.rcx
      disp('ERROR: Could not transfer Signal to RP device');
   end;
   if (invoke(RP,'WriteTagVex','WaveformR',0,'F32',sound(1,:)) == 0) % size is  4e6 samples in Sammelsurium_SoundOutSerSourceStereo.rcx
      disp('ERROR: Could not transfer Signal to RP device');
   end;
...
end % function SoundOutSerSourceStereo