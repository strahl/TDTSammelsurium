function Sammelsurium_SoundOutStereo(RP,TDT_fs)
   if (invoke(RP,'LoadCOFsf','Sammelsurium_SoundOutStereo.rcx',TDT_fs) == 0)
      error('Could not load Sammelsurium_SoundOutStereo.rcx');
   end;
   disp('Sammelsurium_SoundOutStereo.rcx was loaded. Samplingrate was set to 25kHz.');
   if (invoke(RP,'Run') == 0)
   ...
   if (invoke(RP,'WriteTagVex','WaveformL',0,'F32',sound(1,:)) == 0) % size is  4e6 samples in Sammelsurium_SoundOutStereo.rcx
      disp('ERROR: Could not transfer Signal to RP device');
   end;
   if (invoke(RP,'WriteTagVex','WaveformR',0,'F32',sound(1,:)) == 0) % size is  4e6 samples in Sammelsurium_SoundOutStereo.rcx
      disp('ERROR: Could not transfer Signal to RP device');
   end;
